...
支持多种数据源:
- CSV/Excel 本地文件 (CsvFetcher) — 主源，覆盖全、正文完整，零配置
- 卖家精灵 SellerSprite (SellerspriteFetcher) — 可选增强，输入 ASIN 快速预览
  （部分评论正文可能缺失、单 ASIN 覆盖量有限，深度分析建议用 CSV）
...
from src.data_fetchers.sellersprite_fetcher import SellerspriteFetcher

# 可用数据采集器注册表: 名称 -> 实现类
FETCHER_REGISTRY: Dict[str, type] = {
    "csv": CsvFetcher,
    "sellersprite": SellerspriteFetcher,
}
...
def get_fetcher(name: str, **kwargs) -> DataFetcher:
    ...
    Args:
        name: 采集器名称，支持 "csv" 和 "sellersprite"
...
def list_fetchers() -> List[dict]:
    ...
    Returns:
        包含 name, display_name, fields 的字典列表
    """
    result = []
