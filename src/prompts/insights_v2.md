...
以下异常信号已由确定性规则引擎预计算完成，第十三章「异常信号卡」章节需消费此数据渲染决策卡。无异常时为"数据健康"占位文本。
...
3. **Mermaid 图表**：在第5/7/11/14章中按指定格式输出 ```mermaid 代码块。**关键限制：mermaid 代码块内必须全部使用英文**（标题、轴标签、节点名等），中文会导致渲染失败。支持类型：mindmap、flowchart(graph TD/LR)、pie。不支持 quadrantChart。
...
<strategic_json>
{
  ...
  "anomaly_cards": [
    {"type": "异常类型", "severity": "高/中/低", "count": 0, "pct": 0.0, "action": "建议动作"}
  ]
}
</strategic_json>
```
