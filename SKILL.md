...
> **输入文件**：支持本地 CSV/Excel 路径，也支持 HTTP/HTTPS URL（自动下载）。
> **OpenCode 用户**：加 `--engine opencode` 即可切换至 OpenCode CLI 引擎。不指定 `--engine` 时会自动检测环境中可用的 CLI（优先 claude，其次 opencode）。也可设置环境变量 `CLI_ENGINE=opencode` 永久生效。
