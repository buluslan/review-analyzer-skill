...
```bash
# 命令行参数调用方式(非交互环境使用):
# [推荐] 模式 1: CLI 本地模式 (免费, 默认分析 500 条)
python scripts/analyze.py path/to/reviews.csv --max-reviews 100 --mode 1 --creator "AI Assistant"

# 模式 2: 混合模式 (CLI 打标 + Gemini 生成报告和看板, 需 Key)
python scripts/analyze.py path/to/reviews.csv --mode 2 --gemini-key "YOUR_KEY"

# 模式 3: Gemini 高质量模式 (高质量分析, 需 Key)
python scripts/analyze.py path/to/reviews.csv --mode 3 --gemini-key "YOUR_KEY"
```
...
| `--mode` | 运行模式: **1** (CLI本地), **2** (混动模式), **3** (全Gemini) | 1 |
